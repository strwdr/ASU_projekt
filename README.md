...
## Jakub Budrewicz 304018 - Semestr 2022L
## Interpretacja zadania

Należy przygotować skrypt w wybranym języku (Python, Perl, Bash).

Wybrany język - Python

### Funkcjonalności skryptu:
- Jako argumenty podawane są katalogi (X oraz Y)
  - Są to argumenty pozycyjne
- Najpierw należy wybrać jeden z dostępnych trybów z listy:
  1. Find duplicate files
  2. Find empty files
  3. Find temp files
  4. Find same name files
  5. Find bad permission files
  6. Find bad character files
  7. Find X-directory nonexistent 
...
- Przy pierwszym wykrytym pliku jesteśmy pytani o akcję (wybieramy numer z listy dostępnych opcji)
  - Na przykład po wybraniu trybu 5, dostaniemy listę do wyboru:
    1. Automatic chmod (644)
    2. Delete file(s)
    3. Skip
- W zależności od wybranego trybu (trybów) wyświetlane są możliwe różne akcje:
- Plik konfiguracyjny przechowywany jest domyślnie w katalogu `$HOME/.clean_files`


## Uruchamianie
Zakładając, że mamy w lokalizacji skryptu `clean_files.py` foldery `X`, `Y1` oraz `Y2`, w celu uruchomienia skryptu 
należy wykonać polecenie:
```bash
python clean_files.PY X Y1 Y2
```
